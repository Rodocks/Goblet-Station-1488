...
//CHATROOM FUNCTIONS====================================
			...
			if("Set Channel")
				var/t = replacetxt(trim(stripped_input(U, "Please enter channel", name, chat_channel, 15)), " ", "_")
				if(t)
...
/obj/item/device/pda/afterattack(atom/A as mob|obj|turf|area, mob/user, proximity)
	...
	if (!scanmode && istype(A, /obj/item/weapon/paper) && owner)
		if (!A:info)
			...
			return
		notehtml = A:info
		note = replacetxt(notehtml, "<BR>", "\[br\]")
		note = replacetxt(note, "<li>", "\[*\]")
		note = replacetxt(note, "<ul>", "\[list\]")
		note = replacetxt(note, "</ul>", "\[/list\]")
